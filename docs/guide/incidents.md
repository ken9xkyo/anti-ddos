...
- `operator`: gửi test alert và chạy đánh giá ISP runbook.
- `admin`: cấu hình Telegram, bao gồm token write-only, và có toàn bộ quyền operator.

## Thành phần UI

- `Telegram Channel`: trạng thái enabled/disabled, token present/missing, chat id và parse mode.
- Form cấu hình Telegram cho admin: `Bot token`, `Chat ID`, `Parse mode`, `Reason`, `Enabled`.
...
2. Với role admin, cập nhật token/chat/parse mode và nhập `Reason` rõ ràng trước khi `Save config`.
...
- Nếu user không phải admin, form cấu hình Telegram không xuất hiện và có thông báo cần admin role.
...
- Không nhập token Telegram vào nơi khác ngoài form dành cho admin.
