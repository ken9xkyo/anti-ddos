...
- `/v1/udp-source-port-blocks` is the global UDP reflection source-port blocklist with `q`, `state`, and `expiry`; mutation is Operator/Admin, Viewer is read-only, and deletes are soft-disable.
