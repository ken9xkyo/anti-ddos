...
if [[ -n "${ANTI_DDOS_CONTROL_TEST_DSN:-}" ]]; then
  go test ./internal/control -run TestPhase09TelegramAlertingIntegration -count=1
  exit 0
fi

if ! command -v docker >/dev/null 2>&1; then
  echo "docker is required for phase9 PostgreSQL integration test when ANTI_DDOS_CONTROL_TEST_DSN is unset" >&2
  exit 1
fi

name="anti-ddos-phase9-pg-$$"
password="phase9_test_password"
db="anti_ddos_test"
