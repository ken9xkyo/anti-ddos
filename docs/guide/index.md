...
- Disable rule, whitelist và feed là soft-disable. Bản ghi vẫn còn để xem lại lịch sử và không bị xóa vật lý ngay khỏi hệ thống.
...
Các trang CRUD như `Rules`, `Whitelist`, `Snapshots`, `Access` và một số phần của `Reputation` có luồng tải riêng khi mở trang hoặc sau khi thao tác. Điều này giúp overview không phải kéo dữ liệu nặng như raw snapshot trong vòng polling mặc định.
