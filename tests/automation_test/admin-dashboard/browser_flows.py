...
def assert_services_workflow(page: Page, seed: SeedData) -> None:
    ...
    expect(form.get_by_label(re.compile(r"^Name", re.I))).to_be_visible()
    expect(page.get_by_label(re.compile(r"next-hop mac", re.I))).to_have_count(0)
    form.get_by_label(re.compile(r"^Name", re.I)).fill(name)
    form.get_by_label(re.compile(r"^Backend CIDR", re.I)).fill("203.0.113.20/32")
    form.get_by_label(re.compile(r"^Allowed ports", re.I)).fill("443, 8443")
    select_mui_option(page, form, "Output interface", "backend0")
    form.get_by_label(re.compile(r"^Reason", re.I)).fill("automation create UI service")
    ...
    expect(data_grid_row(page, name)).to_be_visible(timeout=15000)
    toolbar.locator("label", has_text="State").locator("select").select_option("disabled")
    expect(page.locator(".MuiDataGrid-row", has_text=name)).to_have_count(0)
    toolbar.locator("label", has_text="State").locator("select").select_option("enabled")

    row = data_grid_row(page, name)
    row.get_by_role("button", name=re.compile(r"^Edit$", re.I)).click()
    form.get_by_label(re.compile(r"^Allowed ports", re.I)).fill("443, 8443, 9443")
    form.get_by_label(re.compile(r"^Reason", re.I)).fill("automation update UI service")
    form.get_by_role("button", name=re.compile(r"Save service", re.I)).click()
    expect_visible_text(page, f"{name} updated", timeout=20000)
    expect(data_grid_row(page, name)).to_contain_text("9443")
    ...
    page.get_by_role("button", name=re.compile(r"Disable service", re.I)).click()
