...
if [[ -n "${ANTI_DDOS_CONTROL_TEST_DSN:-}" ]]; then
  go test ./internal/control -run TestPhase08ThreatFeedSyncIntegration -count=1
  exit 0
fi

if ! command -v docker >/dev/null 2>&1; then
  echo "docker is required for phase8 PostgreSQL integration test when ANTI_DDOS_CONTROL_TEST_DSN is unset" >&2
  exit 1
fi

name="anti-ddos-phase8-pg-$$"
password="phase8_test_password"
db="anti_ddos_test"
